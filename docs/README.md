...
- [ ] 게임 재시작/종료 여부를 입력 (R(재시작)과 Q(종료) 중 하나의 문자를 입력할 수 있으며 올바른 값이 아니면 예외 처리)
...
- [ ] 자동으로 생성할 다리 길이가 3 이상 20 이하의 숫자가 아닌 경우
- [ ] 라운드마다 플레이어가 이동할 칸을 입력이 U(위 칸)와 D(아래 칸) 중 하나의 문자가 아닌 경우
- [ ] 게임 재시작/종료 여부를 입력이 R(재시작)과 Q(종료) 중 하나의 문자가 아닌 경우
